...
p2 <- p + coord_sf(xlim = c(-1.09, -0.945), ylim = c(40.741, 40.894), expand = FALSE) +
  theme(plot.margin = unit(c(0.25, 0.25, 0.25, 0.25), "cm")) +
  #geom_point(data = pancrudo_bound, aes(x = X, y = Y), color = "white", fill = NA, shape = 15, size = 2) +
  theme(plot.background = element_rect(colour = "orange", size = 2.0)) +
  geom_text(data = data.frame(x = -0.999, y = 40.759, label = "Pancrudo"), family = "Helvetica",
            mapping = aes(x = x, y = y, label = label),
            size = 8.5, hjust = 0, vjust = 0, 
            colour = "orange", inherit.aes = FALSE) +
  geom_text(data = data.frame(x = -0.992, y = 40.753, label = "(unknow pleasures)"), 
            family = "Vivaldi", mapping = aes(x = x, y = y, label = label),
            size = 4.4, hjust = 0, vjust = 0, face = "italic",
            colour = "orange", inherit.aes = FALSE)
...
ggsave(p2, filename = here::here("imagenes", "joy_division_Pancrudo_02_naranja_camiseta.png"),
...
knitr::plot_crop(here::here("imagenes", "joy_division_Pancrudo_02_naranja_camiseta.png"))
