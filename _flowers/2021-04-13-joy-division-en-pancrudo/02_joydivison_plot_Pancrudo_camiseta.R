...
p2 <- p + coord_sf(xlim = c(-1.09, -0.945), ylim = c(40.741, 40.894), expand = FALSE) +
  ...
  geom_text(data = data.frame(x = -0.999, y = 40.751, label = "Pancrudo, Teruel"),
            mapping = aes(x = x, y = y, label = label),
            size = 5.8, hjust = 0, vjust = 0, 
            colour = "orange", inherit.aes = FALSE) +
  geom_text(data = data.frame(x = -0.9933, y = 40.76, label = "unknow pleasures in"), family = "Arial",
            mapping = aes(x = x, y = y, label = label),
